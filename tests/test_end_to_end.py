...
Tests everything from initialization to telemetry storage in IoTDB
"""
...
class IoTFlowTester:
    ...
    def test_device_registration(self):
        ...
        try:
            ...
            payload = {
                ...
                "user_id": "a78689be04814ac187e0191656ee6fb1",
            }
            ...
            if response.status_code in [200, 201]:  # Accept both 200 and 201
                ...
                self.log_test(
                    "Device Registration",
                    True,
                    f"Device ID: {self.test_device_id}, Name: {device_name}",
                )
                return True
            else:
                self.log_test(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test(
                    ...
                    True,
                    f"Device authenticated: {device_info.get('name')}",
                )
                return True
            else:
                self.log_test(
        ...
        try:
            ...
            if response.status_code not in [200, 201]:  # Accept both 200 and 201
                self.log_test(
                    ...
                    False,
                    f"Set config failed: HTTP {response.status_code}",
                )
                return False
            ...
            if response.status_code == 200:
                ...
                self.log_test(
                    "Device Configuration",
                    True,
                    f"Config set and retrieved: {len(config)} items",
                )
                return True
            else:
                self.log_test(
        ...
        try:
            ...
            if response.status_code in [200, 201]:  # Accept both 200 and 201
                ...
                self.log_test(
                    ...
                    True,
                    f"Telemetry stored: {data.get('message')}",
                )
                return True
            else:
                self.log_test(
    ...
    def test_admin_device_details(self):
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/api/v1/admin/devices/{self.test_device_id}",
                headers=headers,
            )
    ...
    def test_iotdb_verification(self):
        """Test 12: Verify data is stored in IoTDB (via telemetry status check)"""
        try:
            # Check IoTDB status endpoint
            response = self.session.get(f"{self.base_url}/api/v1/telemetry/status")
            if response.status_code != 200:
                self.log_test("IoTDB Verification", False, "Failed to get IoTDB status")
                return False

            status_data = response.json()
            iotdb_available = status_data.get("iotdb_available", False)

            if not iotdb_available:
                self.log_test("IoTDB Verification", False, "IoTDB is not available")
                return False

            # Send verification telemetry with API key
            headers = {
                "Content-Type": "application/json",
                "X-API-Key": self.test_api_key,
            }
            ...
            telemetry_payload = {
                ...
                "metadata": {
                    "test_purpose": "iotdb_verification",
            ...
            if response.status_code not in [200, 201]:
                self.log_test(
                    "IoTDB Verification",
                    False,
                    f"Failed to send verification telemetry: {response.status_code}",
                )
                return False

            # Check that telemetry was stored in IoTDB
            response_data = response.json()
            stored_in_iotdb = response_data.get("stored_in_iotdb", False)

            if stored_in_iotdb:
                self.log_test(
                    "IoTDB Verification",
                    True,
                    f"Telemetry successfully stored in IoTDB for device {self.test_device_id}",
                )
                return True
            else:
            ...
            self.log_test("IoTDB Verification", False, str(e))
            return False
