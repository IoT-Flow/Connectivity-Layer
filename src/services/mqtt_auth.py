...
class MQTTAuthService:
    ...
    def authenticate_device_by_api_key(self, api_key: str) -> Optional[Device]:
        ...
        try:
            with self.app.app_context():
                ...
                logger.info(
                    "Authenticating device %d (%s) with status: %s",
                    device.id,
                    device.name,
                    device.status,
                )
    ...
    def validate_device_message(
        self, device_id: int, api_key: str, topic: str
    ) -> Optional[Device]:
        """
        ...
        try:
            ...
            if not self.is_device_authorized(device_id, topic):
                logger.warning(
                    "Device %d not authorized for topic: %s", device_id, topic
                )
                return None
    ...
    def is_device_authorized(self, device_id: int, topic: str) -> bool:
        ...
        try:
            ...
            if not device:
                ...
                logger.info(
                    "Device %d (%s) authorized for topic: %s",
                    device_id,
                    device.name,
                    topic,
                )
    ...
    def handle_telemetry_message(
        ...
        try:
            with self.app.app_context():
                ...
                is_registered, reg_message = self.validate_device_registration(
                    device_id, api_key
                )
                ...
                try:
                    ...
                    logger.error(
                        "Invalid JSON payload from device %d: %s", device_id, payload
                    )
                    return False
                ...
                if not device:
                    logger.warning(
                        "Unauthorized telemetry attempt from device_id %d", device_id
                    )
                    return False
                ...
                if not telemetry_data:
                    logger.warning(
                        "No telemetry data extracted from message for device %d",
                        device_id,
                    )
                    return False
                ...
                logger.debug(
                    f"Processing telemetry for device {device_id}: {telemetry_data}"
                )
                ...
                if (
                    ...
                    logger.error(
                        "Failed to store telemetry in IoTDB for device %d", device_id
                    )
                    return False
    ...
    def cleanup_inactive_devices(self):
        ...
        try:
            ...
            logger.info(
                f"Cleaned up inactive devices. Active: {len(active_device_ids)}"
            )
    ...
    def validate_device_registration(
        self, device_id: int, api_key: str
    ) -> tuple[bool, str]:
        """
        ...
        try:
            with self.app.app_context():
                ...
                if not device:
                    logger.warning(
                        f"Device registration validation failed: Device {device_id} not found with provided API key"
                    )
                ...
                logger.info(
                    f"Device registration validation successful for device {device_id} (status: {device.status})"
                )
    ...
    def is_device_registered_for_mqtt(
        self, payload: dict
        ...
        try:
            ...
            logger.info(
                f"Device {device.id} ({device.name}) authorized for MQTT communication"
            )
