...
class TelemetryMessageHandler(MQTTMessageHandler):
    ...
    def handle_message(self, message: MQTTMessage) -> None:
        ...
        try:
            ...
            if (
                len(topic_parts) < 4
                or topic_parts[0] != "iotflow"
                or topic_parts[1] != "devices"
                or topic_parts[3] != "telemetry"
            ):
            ...
            if self.auth_service:
                ...
                if not success:
                    self.logger.warning(
                        "Authentication failed or telemetry storage failed for device %d",
                        device_id,
                    )
                    return
            else:
        ...
        try:
            ...
            for topic_pattern, callbacks in self.subscription_callbacks.items():
                if self._topic_matches_pattern(message.topic, topic_pattern):
                    self.logger.debug(
                        f"Topic {message.topic} matches pattern {topic_pattern}, calling {len(callbacks)} callbacks"
                    )
